...
class Squeeze_RedConv(nn.Module):
    def __init__(self):
        super(Squeeze_RedConv, self).__init__()

        self.conv1 = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=3, stride=2, padding=0, bias=False)
        self.conv_red1 = nn.Conv2d(in_channels=16, out_channels=8, kernel_size=1, stride=1, padding=0)
        self.acff1 = ACFF(8, 64)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.acff2 = ACFF(64, 96)
        self.conv_red2 = nn.Conv2d(in_channels=96, out_channels=48, kernel_size=1, stride=1, padding=0)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.acff3 = ACFF(48, 128)
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv_red3 = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=1, stride=1, padding=0)
