...
def load_onnx(model_name):
    """Read the ONNX file."""
    onnx_path = 'onnx/%s.onnx' % model_name
...
def build_engine(model_name, do_int8, dla_core, verbose=False, fp32=False):
...
def main():
    ...
    engine = build_engine(
        args.model, args.int8, args.dla_core, args.verbose, args.fp32)
    if engine is None:
        raise SystemExit('ERROR: failed to build the TensorRT engine!')

    engine_path = 'engines/%s.trt' % args.model
